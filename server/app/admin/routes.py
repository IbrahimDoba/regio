...
"""DASHBOARD"""
@router.get("/stats", response_model=SystemStats, status_code=status.HTTP_200_OK, responses={
...
"""USER MANAGEMENT"""
@router.get("/users", response_model=UserListResponse, status_code=status.HTTP_200_OK, responses={
    status.HTTP_500_INTERNAL_SERVER_ERROR: {"description": "Internal server error."}
})
...
@router.patch("/users/{user_code}", response_model=UserPublic, status_code=status.HTTP_200_OK, responses={
    status.HTTP_404_NOT_FOUND: {"description": "User not found."},
    status.HTTP_403_FORBIDDEN: {"description": "Action not permitted on this user."},
    status.HTTP_500_INTERNAL_SERVER_ERROR: {"description": "Internal server error."}
})
...
@router.patch("/users/{user_code}/toggle", response_model=UserPublic, status_code=status.HTTP_200_OK, responses={
    status.HTTP_404_NOT_FOUND: {"description": "User not found."},
    status.HTTP_403_FORBIDDEN: {"description": "Cannot ban this user (e.g. another admin)."},
    status.HTTP_500_INTERNAL_SERVER_ERROR: {"description": "Internal server error."}
})
...
"""TAG MANAGEMENT"""
@router.get("/tags", response_model=List[TagAdminView], status_code=status.HTTP_200_OK, responses={
    status.HTTP_500_INTERNAL_SERVER_ERROR: {"description": "Internal server error."}
})
async def get_tags(
    service: AdminServiceDep,
...
@router.patch("/tags/{tag_id}", status_code=status.HTTP_200_OK, responses={
...
@router.delete("/tags/{tag_id}", status_code=status.HTTP_204_NO_CONTENT, responses={
    status.HTTP_404_NOT_FOUND: {"description": "Tag not found."},
    status.HTTP_500_INTERNAL_SERVER_ERROR: {"description": "Internal server error."}
})
...
"""BANKING / DISPUTES"""
@router.get("/disputes", response_model=List[DisputePublic], status_code=status.HTTP_200_OK, responses={
    status.HTTP_500_INTERNAL_SERVER_ERROR: {"description": "Internal server error."}
})
...
@router.post("/disputes/{request_id}/resolve", status_code=status.HTTP_200_OK, responses={
    status.HTTP_400_BAD_REQUEST: {"description": "Invalid action or payment state."},
