...
class AdminService:
    ...
    async def get_system_stats(self) -> SystemStats:
        ...
        verification_pending_users = await self.session.execute(select(func.count(User.id)).where(User.verification_status == VerificationStatus.PENDING))
    ...
    # USER MANAGEMENT
    async def get_users_rich(self, query: str = None, skip: int = 0, limit: int = 20) -> UserListResponse:
        """
    ...
    # TAG MANAGEMENT
    async def get_tags_with_usage(self, pending_only: bool = False) -> List[TagAdminView]:
        """
        ...
        stmt = select(Tag)
        if pending_only:
            stmt = stmt.where(Tag.is_official == False)
        else:
            stmt = stmt.where(Tag.is_official == True)
    ...
    async def send_broadcast(self, data: BroadcastCreate):
        ...
        count_res = await self.session.execute(select(func.count()).select_from(stmt.subquery()))
