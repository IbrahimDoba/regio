...
class ProductAttributes(BaseModel):
    ...
    def check_price_exists(self) -> 'ProductAttributes':
        if self.regio_amount is None and self.time_amount is None:
            raise ValueError("Product must have a price (either regio_amount or time_amount)")
        return self

class RentalAttributes(BaseModel):
    fee_regio: Optional[int] = Field(None, description="Rental fee in Regio per unit of time.")
    fee_time: Optional[int] = Field(None, description="Rental fee in Time per unit of time.")
...
class RideShareAttributes(BaseModel):
    start: str = Field(..., min_length=2, description="Starting location (City or Address).")
    destination: str = Field(..., min_length=2, description="Destination (City or Address).")

class EventAttributes(BaseModel):
    event_start_date: datetime = Field(..., description="ISO 8601 format start date and time of the event.")
    event_end_date: datetime = Field(..., description="ISO 8601 format end date and time of the event.")
...
class CreateServiceListing(ListingCreateBase):
    category: Literal[ListingCategory.OFFER_SERVICE] = Field(ListingCategory.OFFER_SERVICE, description="Category: OFFER_SERVICE")
...
class CreateProductListing(ListingCreateBase):
    category: Literal[ListingCategory.SELL_PRODUCT] = Field(ListingCategory.SELL_PRODUCT, description="Category: SELL_PRODUCT")
    attributes: ProductAttributes = Field(..., description="Pricing information (Regio or Time).")
...
class CreateRentalListing(ListingCreateBase):
    category: Literal[ListingCategory.OFFER_RENTAL] = Field(ListingCategory.OFFER_RENTAL, description="Category: OFFER_RENTAL")
...
class CreateRideShareListing(ListingCreateBase):
    category: Literal[ListingCategory.RIDE_SHARE] = Field(ListingCategory.RIDE_SHARE, description="Category: RIDE_SHARE")
...
ListingCreate = Annotated[
    ...
    Field(discriminator='category', description="Polymorphic Listing Creation. Select the schema matching your category.")
]
