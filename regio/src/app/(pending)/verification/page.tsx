...
export default function VerificationPage() {
  ...
  return (
    <MobileContainer className="flex flex-col bg-[#fcfcfc] bg-[url('https://images.unsplash.com/photo-1621360841013-c768371e93cf?q=80&w=1000&auto=format&fit=crop')] bg-cover bg-center bg-blend-multiply">
      ...
      <div className="m-[20px] bg-white rounded-[20px] p-[30px_25px] text-center shadow-lg animate-in fade-in slide-in-from-bottom-4 duration-600">
        {isVerified ? (
          <div className="w-[80px] h-[80px] bg-[#f0f7e6] rounded-full mx-auto mb-[20px] flex justify-center items-center border-[2px] border-[var(--color-green-offer)] shadow-[0_0_0_6px_rgba(140,179,72,0.15)]">
            <FaCircleCheck className="text-[36px] text-[var(--color-green-offer)]" />
          </div>
        ) : (
        ...
        <h1 className="text-[22px] font-[800] text-[#333] mb-[10px]">
          {isVerified ? t.verification.title_verified : t.verification.title}
        </h1>
        <p className="text-[14px] text-[#666] leading-[1.5] mb-[30px]">
          {isVerified ? t.verification.description_verified : t.verification.description}
        </p>
        ...
        <div className="flex justify-between items-center relative mb-[40px] px-[10px]">
          ...
          <div className="relative z-10 flex flex-col items-center gap-[8px] w-1/3">
            ...
            <span className="text-[11px] font-[600] text-[var(--color-green-offer)]">{t.verification.step_register}</span>
          </div>
          ...
          <div className="relative z-10 flex flex-col items-center gap-[8px] w-1/3">
            ...
            <span className={`text-[11px] font-[${isVerified ? '600' : '800'}] ${isVerified ? 'text-[var(--color-green-offer)]' : 'text-[#f57c00]'}`}>
              {t.verification.step_video_call}
            </span>
          </div>
          ...
          <div className="relative z-10 flex flex-col items-center gap-[8px] w-1/3">
            {isVerified ? (
              <div className="w-[32px] h-[32px] rounded-full bg-[var(--color-green-offer)] border-[2px] border-[var(--color-green-offer)] flex justify-center items-center text-white text-[12px] animate-pulse shadow-[0_0_0_4px_rgba(140,179,72,0.2)]">
                <FaCheck />
              </div>
            ) : (
            ...
            <span className={`text-[11px] font-[${isVerified ? '800' : '600'}] ${isVerified ? 'text-[var(--color-green-offer)]' : 'text-[#999]'}`}>
              {t.verification.step_active}
            </span>
          </div>
        </div>
        ...
        {isVerified && (
          <button
            className="w-full p-[12px] bg-[var(--color-green-offer)] text-white border-none rounded-[6px] text-[14px] font-[700] cursor-pointer flex justify-center items-center gap-[8px] hover:scale-[1.02] transition-transform mb-[20px]"
            onClick={() => router.push('/')}
          >
            {t.verification.go_to_app}
          </button>
        )}

        <div className="text-[11px] text-[#999] mt-[20px]">
          <FaLock className="inline mr-[4px]" /> <span>{t.verification.data_notice}</span>
        </div>
      </div>
