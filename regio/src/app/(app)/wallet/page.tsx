    ...
    <div className="relative">
      ...
      {selectedCode && (
        <div className="text-[10px] text-[#888] mt-[3px]">
          {t.wallet.autocomplete_selected} <span className="font-mono font-[700]">{selectedCode}</span>
        </div>
      )}
        ...
        <div className="absolute left-0 right-0 top-[42px] bg-white border border-[#ddd] rounded-[4px] shadow-lg z-[20] max-h-[190px] overflow-y-auto">
          {isFetching && (
            <div className="px-[10px] py-[9px] text-[12px] text-[#888]">
              {t.wallet.autocomplete_searching}
            </div>
          )}
          {!isFetching && users.length === 0 && (
            <div className="px-[10px] py-[9px] text-[12px] text-[#888]">
              {t.wallet.autocomplete_no_results}
            </div>
          )}
          {users.map((user) => (
            <button
...
export default function WalletPage() {
  ...
  const handleSend = async () => {
    ...
    if (!sendRecipient.trim()) {
      setSendError(t.wallet.error_recipient_required);
      return;
    }
    ...
    if (!hasGaras && !hasTime) {
      setSendError(t.wallet.error_amount_required);
      return;
    }
    if (!sendRef.trim()) {
      setSendError(t.wallet.error_reference_required);
      return;
    }

    if (!await dialog.confirm(t.wallet.confirm_transfer_title, t.wallet.confirm_transfer_body.replace("{recipient}", sendRecipient))) return;
    ...
    transfer.mutate(
      ...
      },
      {
        ...
        onError: (err) => {
          setSendError(getErrorMessage(err, t.wallet.error_unexpected));
        },
      }
    );
  };
  ...
  const handleCreateRequest = () => {
    ...
    if (!reqUser.trim()) {
      setReqError(t.wallet.error_user_required);
      return;
    }
    ...
    if (!hasGaras && !hasTime) {
      setReqError(t.wallet.error_amount_required);
      return;
    }
    if (!reqDesc.trim()) {
      setReqError(t.wallet.error_description_required);
      return;
    }
    ...
    createRequest.mutate(
      ...
      },
      {
        ...
        onError: (err) => {
          setReqError(getErrorMessage(err, t.wallet.error_unexpected));
        },
      }
    );
  };
  ...
  const handleConfirmRequest = async (id: string, amount: string) => {
    setActionError(null);
    if (!await dialog.confirm(t.wallet.confirm_payment_title, t.wallet.confirm_payment_body.replace("{amount}", amount))) return;
    confirmRequest.mutate(id, {
      onSuccess: () => {
        refetchIncoming();
      },
      onError: (err) => {
        setActionError({ id, msg: getErrorMessage(err, t.wallet.error_unexpected) });
      },
    });
  };
  ...
  const handleRejectRequest = async (id: string) => {
    setActionError(null);
    if (!await dialog.confirm(t.wallet.decline_request_title, t.wallet.decline_request_body)) return;
  ...
  const handleCancelRequest = async (id: string) => {
    setActionError(null);
    if (!await dialog.confirm(t.wallet.cancel_request_title, t.wallet.cancel_request_body)) return;
  ...
  const handleRaiseDispute = async (id: string) => {
    const reason = await dialog.prompt(
      t.wallet.dispute_title,
      t.wallet.dispute_body,
      t.wallet.dispute_placeholder
    );
    ...
    <div className="bg-[var(--bg-app)] min-h-screen pb-[70px]">
      ...
      {totalRequestsVisible > 0 && (
        <div className="px-[15px] mb-[10px]">
          ...
          {rejectedOutgoing.map((req) => (
            ...
            >
              <div className="flex justify-between mb-[5px] text-[11px] text-[#666]">
                <span className="text-[#e53935] font-semibold">{t.wallet.declined_request}</span>
                <span>{new Date(req.created_at).toLocaleDateString()}</span>
              </div>
              <div className="flex justify-between items-center mb-[10px]">
                <div>
              ...
              <div className="flex gap-[10px] mt-[8px]">
                ...
                {req.dispute_raised ? (
                  ...
                  >
                    {raiseDisputeMutation.isPending ? "..." : t.wallet.raise_dispute_button}
                  </button>
                )}
              </div>
            </div>
      ...
      >
        <div className="p-[20px]">
          ...
          {sendSuccess && <InlineSuccess message={t.wallet.transfer_success} />}
      ...
      >
        <div className="p-[20px]">
          ...
          {reqSuccess && <InlineSuccess message={t.wallet.request_success} />}
          ...
          <div className="w-[85%] max-w-[400px] bg-white rounded-[12px] p-[20px] relative shadow-2xl text-center animate-in zoom-in-95">
            ...
            <div className="text-[13px] text-[#888] mb-[20px]">
              ...
              {selectedTx.is_system_fee && (
                <div className="mt-[4px] text-[11px] text-[#7b1fa2]">{t.wallet.system_charge}</div>
              )}
            </div>
