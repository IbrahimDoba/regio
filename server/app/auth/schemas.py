...
class InvitePublic(SQLModel):
    ...
    uses_left: int = Field(
        ..., description="Number of times this code can still be used."
    )
    expires_at: Optional[str] = Field(
        default=None,
        description="ISO timestamp when the invite expires (currently not used).",
    )

    # Tracking info
    is_used: bool = Field(
        ..., description="True if the code has been fully consumed/depleted."
    )
    used_by_name: Optional[str] = Field(
        default=None,
        description="Display name of the user who used this code (if applicable).",
    )
