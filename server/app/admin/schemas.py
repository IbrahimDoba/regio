...
class TagAdminView(BaseModel):
    ...
    usage_count: int = Field(
        default=0, description="Number of listings currently using this tag."
    )
...
class TagsAdminListResponse(BaseModel):
    ...
    count: int = Field(
        ..., description="Total matching tags (for pagination)."
    )
...
class DisputeAction(BaseModel):
    ...
    reason: Optional[str] = Field(
        default=None, description="Administrative note for the resolution."
    )
...
class BroadcastCreate(BaseModel):
    ...
    target_audience: str = Field(
        ..., description="Target group key (e.g. 'ALL', 'VERIFIED')."
    )
