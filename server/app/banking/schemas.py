...
from app.banking.enums import PaymentStatus, TransactionType
...
class AccountLimits(SQLModel):
    ...
    max_debt_time: int = Field(
        ..., description="Maximum allowed negative balance for Time."
    )
    max_debt_regio: Decimal = Field(
...
class BalanceResponse(SQLModel):
    ...
    trust_level: TrustLevel = Field(
        ..., description="Current trust level (determines limits)."
    )
    total_time_earned: int = Field(
        ..., description="Lifetime minutes earned (Reputation score)."
    )
...
class TransactionPublic(SQLModel):
    ...
    type: TransactionType = Field(
        ..., description="Type: TRANSFER, SYSTEM_FEE, DEMURRAGE, etc."
    )

    other_party_code: str = Field(
        ..., description="User code of the counterparty (Sender or Receiver)."
    )
...
class PaymentRequestPublic(SQLModel):
    ...
    description: Optional[str] = Field(
        default=None, description="Reason for the request."
    )

    status: PaymentStatus = Field(
        ..., description="PENDING, APPROVED, REJECTED, CANCELLED."
    )
    dispute_raised: bool = Field(default=False, description="True if the creditor has contested the rejection.")
    dispute_reason: Optional[str] = Field(default=None, description="Creditor's reason for disputing.")
    dispute_raised_at: Optional[datetime] = Field(default=None, description="When the creditor raised the dispute.")
...
class DemurrageResult(SQLModel):
    ...
    total_minutes_collected: int = Field(
        ..., description="Total minutes removed from circulation."
    )
